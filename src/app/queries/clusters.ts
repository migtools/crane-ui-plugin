...
const clusterResource = new MigResource(MigResourceKind.MigCluster, 'openshift-migration');
...
export const useClustersQuery = (): UseQueryResult<IKubeList<any>> => {
  ...
  const result = useMockableQuery<IKubeList<any>>(
    {
      queryKey: 'clusters',
      queryFn: async () => (await client.list<IKubeList<any>>(clusterResource)).data,
      refetchInterval: 5000,
      select: sortKubeListByNameCallback,
    },
    mockKubeList([], 'Cluster')
  );
...
export const useCreateClusterMutation = (
  ...
  return useMockableMutation<IKubeResponse<any>, KubeClientError, MockPlanFormState>(
    async (forms) => {
      ...
      const clusterResponse = await client.create<any>(clusterResource, mockPlan);
